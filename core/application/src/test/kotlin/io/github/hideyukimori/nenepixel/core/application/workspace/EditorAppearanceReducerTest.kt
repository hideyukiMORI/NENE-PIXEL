        ...
        EditorTheme.entries.forEach { theme ->
            EditorLayout.entries.forEach { layout ->
                EditorControlEdge.entries.forEach { edge ->
                    ...
                    val next = reducer.reduce(initial, WorkspaceAction.SetAppearance(appearance), admission).nextState
                    ...
                    assertEquals(
                        next,
                        reducer.reduce(initial, WorkspaceAction.SetAppearance(appearance), admission).nextState,
                    )
                }
            }
        }
    ...
    fun `repeated appearance preserves state identity but cancels an active preview`() {
        ...
        val reduced =
            assertInstanceOf(
                ...
                reducer.reduce(preview, action, admission),
            )
