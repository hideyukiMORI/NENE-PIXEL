...
private class FakeProjectStoragePort : ProjectStoragePort {
...
private class FakeRecoveryRecordPort(
...
private suspend fun initializedFixture(): Fixture = Fixture().also { it.initialize() }
...
private fun assertFailedInspection(result: RecoveryInitializationResult): RecoveryInspectionFailure =
    assertInstanceOf(RecoveryInitializationResult.Failed::class.java, result).failure

private fun assertSaved(result: PersistenceRequestResult): PersistenceLastOutcome.Saved =
    assertInstanceOf(
        PersistenceLastOutcome.Saved::class.java,
        assertInstanceOf(PersistenceRequestResult.Completed::class.java, result).outcome,
    )

private fun assertRetiredCleanup(cleanup: RecoveryCleanupOutcome): RecoveryGeneration =
    assertInstanceOf(RecoveryCleanupOutcome.Retired::class.java, cleanup).generation

private fun assertFailed(result: PersistenceRequestResult): PersistenceFailure =
    assertInstanceOf(
...
private fun assertAwaiting(result: PersistenceRequestResult): PersistenceConfirmationRequest =
    assertInstanceOf(PersistenceRequestResult.AwaitingConfirmation::class.java, result).request

private fun assertSaving(phase: PersistenceOperationPhase): PersistenceOperationHandle =
    assertInstanceOf(PersistenceOperationPhase.Saving::class.java, phase).operation

private fun assertLoading(phase: PersistenceOperationPhase): PersistenceOperationHandle =
    assertInstanceOf(PersistenceOperationPhase.Loading::class.java, phase).operation

private fun assertSwitching(phase: PersistenceOperationPhase): PersistenceOperationHandle =
