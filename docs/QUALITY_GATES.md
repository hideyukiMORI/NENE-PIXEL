...
match the canonical UTF-8/LF bytes while preserving the complete ordered rule and flag content.
Refreshing that artifact is a separate explicit, evidence-preserving physical-device operation
defined by ADR 0010. A failed or nonfresh generation must not pass by reusing an old source file.
