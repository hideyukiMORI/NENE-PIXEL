...
production implementation. It is based on independent review of the current Issue, ADR 0018 and
...
Protocol identity: `nene-pixel-p4-indexed-cutover-verification-v1`.
...
timed samples. Cheap post-timer facts are result kind, exact history position
transition, revision, palette transition kind/default, ChangeSet revisions,
invalidation, and no-op owner identity. ART snapshots are outside the direct
timer as in the current collector.
...
sequentially while proving each old preview is released, and takes post-cycle
post-GC. Full owner inventories occur only at fixed checkpoints.
...
Only the immutable source, destination, identity and transport fixtures are
created outside the timer. Production classification/reduction and every
mutable planning-workspace allocation they perform are inside the timed
operation; the evidence harness may not preallocate or inject that private
workspace. Cheap checks are result kind, exact count, output
size/palette/default, operation/source identity and destination epoch. Full
RGBA/reference comparison is outside the population. There is no baseline
