...
function Invoke-GradleCommand {
    ...
    $arguments = @{
        ...
        LogPath = $LogPath
        ExecutablePath = Join-Path $RepositoryRoot 'gradlew.bat'
        NativeArguments = @($GradleArguments) + @('--no-daemon')
        TimeoutSeconds = $TimeoutSeconds
    }
    if ($PSBoundParameters.ContainsKey('JobTerminator')) { $arguments.JobTerminator = $JobTerminator }
    Invoke-BoundedNativeCommand @arguments
}
